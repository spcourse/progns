...
In Python gebruik je hiervoor de optie `plt.hist()` om de data te groeperen en laat het dan pas zien m.b.v. `plt.show()` of `plt.savefig()`. Je kunt bij het groeperen opgeven in hoeveel stukjes (bins) je de data op wilt delen.
...
Schrijf een functie `som_random_getallen()` die de distributie weergeeft van 10,000 experimenten. Geef de resultaten weer tussen x = 30 en x = 70.
